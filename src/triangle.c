...
{
    // Calculating the change in x over y. That is why is inverted
    ...
    // Loop all scanelines from top to bottom
    for (int y = y0; y <= y2; y++)
    {
...
void fill_flat_top_triangle(int x0, int y0, int x1, int y1, int x2, int y2, uint32_t color)
{
    ...
    // Loop all scanelines from bottom to top
    for (int y = y2; y >= y0; y--)
    {
        draw_line(x_start, y, x_end, y, color);
        x_start -= inv_slope_1;
...
void draw_filled_triangle(int x0, int y0, int x1, int y1, int x2, int y2, uint32_t color)
{
    ...
    if (y1 == y2)
    {
        // We simply draw the flat-bottom triangle
        fill_flat_bottom_triangle(x0, y0, x1, y1, x2, y2, color);
    }
    else if (y0 == y1)
    {
        // We simply draw the flat-top triangle
    ...
    tex2_t a_uv, tex2_t b_uv, tex2_t c_uv)
{
    ...
    // Variables to store the interpolated of U, V and also 1/W for the current pixel
    ...
    // Preventing to exced the size of the texture
    int pos = ((texture_width * tex_y) + tex_x) % (texture_width * texture_height);
    draw_pixel(x, y, texture[pos]);
}
